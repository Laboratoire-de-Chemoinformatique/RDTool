...
import org.openscience.smsd.filters.PostFilterNew;
...
//        if (DEBUG) {
//            System.out.println("Prefilter.filter:");
//            for (List<Integer[]> m : mappings) {
//                System.out.println("Next:");
//                for (Integer[] p : m) System.out.println("\t" + p[0] + ", " + p[1]);
//                System.out.println("-----");
//            }
//        }
        List<Map<Integer, Integer>> solutions = PostFilterNew.filter(mappings);
//        if (DEBUG) {
//            System.out.println("Postfilter.filter:");
//            for (Map<Integer, Integer> set : solutions) {
//                System.out.println("Next:");
//                for (Map.Entry<Integer, Integer> p : set.entrySet()) System.out.println("\t" + p.getKey() + ", " + p.getValue());
//                System.out.println("-----");
//            }
//        }
        ...
        try {
            allMCS.stream().map((solution) -> {
                ...
                solution.entrySet().stream().forEach((m) -> {
                    ...
                    if (DEBUG) System.out.println("\tindexI = " + indexI + ", indexJ = " + indexJ);
    ...
    private List<List<Integer[]>> reverseMappings(List<List<Integer[]>> mappings) {
        ...
        mappings.stream().map((mapping) -> {
            List<Integer[]> new_mapping = new ArrayList<>();
            for (int i = mapping.size() - 1; i >= 0; i--) {
                Integer[] entry = mapping.get(i);
                new_mapping.add(new Integer[]{entry[1], entry[0]});
            }
            return new_mapping;
        }).forEach(reverse::add);
        return reverse;
